...
local whitelistedHWIDs = {
    "4a07e4b5-817e-4f8f-9e75-2b6a6ff8b511",
    "5dd833a5-6236-4f57-9c55-64371b5dc22e",
    "e12b73f8-d848-466e-85b8-052259df7eae",
    "8ef312b6-dc89-4427-a1fb-bd6def9ce84e",
    "5e6caf4c-aa15-4e16-8c32-b1db754c6c6e",
    "256f5cd3-0794-40b9-a416-eb0b80472e4e",
    "d674109b-f3fa-4ce6-afd2-ceaf7b95c05b",
    "ccfd4ab9-f255-4ec5-aa40-df74f0a8306f",
    "aa12c426-a6e8-4f0a-9455-cf2cf32d28ad",
    "e079400a-84ab-4512-bd4c-73db941f2dab",
    "HWID11",
    "HWID12",
    "HWID13"
}
...
ImageLabel.Image = "rbxassetid://18689675910"
...
-- Add the declaration for ToggleText
local ToggleText = Instance.new("TextLabel")
ToggleText.Parent = ScreenGui
ToggleText.Text = "Loaded"
ToggleText.Size = UDim2.new(0, 100, 0, 50)
ToggleText.Position = UDim2.new(0.5, -50, 0.5, -25)
    ...
    function()
        if getgenv().Speed then
            lplr.Character.HumanoidRootPart.CFrame =
                lplr.Character.HumanoidRootPart.CFrame + lplr.Character.Humanoid.MoveDirection * getgenv().SpeedAmount
        end
    end
)
...
local function OnButtonClick()
    ...
    button.Text = Toggle and "Stop" or "360"
end
...
print("loaded")
warn("doesnt have resolver BITCH")
...
-- Unpack settings for convenience
local Enabled = Settings.electron.Enabled
local Prediction = Settings.electron.Prediction
local Notifications = Settings.electron.Notifications
local AimPart = Settings.electron.AimPart
local Kocheck = Settings.electron.KoCheck
local LookAt = Settings.electron.LookAt
local AutoAir = Settings.electron.AutoAir
local Resolver = Settings.electron.Resolver
local Highlight = Settings.electron.Highlight
local Tool = Settings.Global.Tool_Lock 
local Button = Settings.Global.Button 

local SmoothCamSpeed = Settings.Camera.SmoothCamSpeed
local ShakeIntensity = Settings.Camera.ShakeIntensity

local NoFog = Settings.Visuals.No_Fog
local ColorCorrection = Settings.Visuals.Color_Correction
local Ambient = Settings.Visuals.Ambient
local LowGFX = Settings.Visuals.LowGFX
local Headless = Settings.Visuals.Headless
local Korblox = Settings.Visuals.Korblox

local Black = Settings.Textures.Black
local Regular = Settings.Textures.Regular
local Sand = Settings.Textures.Sand

local TTKO = Settings.Misc.TrashTalk_On_KO

local Macro = Settings.SpeedGlitch.Macro
local Respawn = Settings.SpeedGlitch.Respawn

local fovSettings = getgenv().Settings.FOV
local fovCircle = getgenv().Settings.FOVconfig.circle
local silentAim = getgenv().Settings.FOVconfig.silentAim

fovCircle.Color = fovSettings.color
fovCircle.Visible = fovSettings.showFOV
fovCircle.Filled = fovSettings.fill
...
Part.Color = Color3.fromRGB(0, 0, 139)
...
Electron.Parent = game.CoreGui
Electron.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

local Frame = Instance.new("Frame")
Frame.Parent = Electron
Frame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
Frame.BorderColor3 = Color3.fromRGB(0, 0, 0)
Frame.BorderSizePixel = 0
Frame.Position = UDim2.new(0.1338, 0, 0.2011, 0)
...
Logo.Image = "rbxassetid://18689675910"
...
TextButton.Text = "Toggle Electron"
TextButton.TextColor3 = Color3.fromRGB(255, 255, 255)
...
-- Dear bruce i need to be honest n clear

game:GetService("RunService").Stepped:Connect(
    function()
        if Enabled then
            Part.Position = game.Players[TargetPlayer].Character.HumanoidRootPart.Position
            -- Smooth lock functionality
            Camera.CFrame =
                Camera.CFrame:Lerp(
                CFrame.new(Camera.CFrame.Position, game.Players[TargetPlayer].Character[AimPart].Position),
                SmoothCamSpeed
            )
            ...
            Camera.CFrame =
                Camera.CFrame *
                CFrame.Angles(
                    math.random(-ShakeIntensity, ShakeIntensity) * 0.01,
                    math.random(-ShakeIntensity, ShakeIntensity) * 0.01,
                    0
                )
        else
...
game:GetService("RunService").Stepped:Connect(
    function()
        if Highlight then
            ...
            LocalHL.Parent = TargetPlayer
            LocalHL.FillTransparency = 1
...
local function toggleSky()
    ...
    ToggleButton.BackgroundColor3 = getgenv().Sky and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 0, 0)
...
ToggleButton.MouseButton1Click:Connect(toggleSky)
...
game:GetService("RunService").RenderStepped:Connect(
    function()
        if
            Enabled and Kocheck and
                (game.Players[TargetPlayer].Character.Humanoid.Health <= 2.25 or
                    game.Players.LocalPlayer.Character.Humanoid.Health <= 2.25)
         then
            Enabled = false
        end
    end
...
game:GetService("RunService").Stepped:Connect(
    function()
        if Enabled and Lookat then
            local localPlayer = game.Players.LocalPlayer
            local targetPlayer = game.Players:FindFirstChild(TargetPlayer)
...
game:GetService("RunService").RenderStepped:Connect(function()
    if TTKO and (game.Players[TargetPlayer].Character.Humanoid.Health <= 2) then
        game.ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("tap in w electron lil bro", "All")
        wait(0.6)
        game.ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("ur dirt btw", "All")
    end
end)
...
game:GetService("RunService").RenderStepped:Connect(
    function()
        if Ambient then
            game.Lighting.OutdoorAmbient = Color3.fromRGB(0, 0, 139)
        end
    end
)

-- now your the only one keeping me awake .

game.Players.LocalPlayer.CharacterAdded:Connect(
    function()
        loadstring(
            game:HttpGet("https://raw.githubusercontent.com/DHBCommunity/DHBOfficialScript/main/macromobbyballigusapo")
        )()
    end
)
...
-- hold on tight this time we get cause
                    ...
                    if part:IsA("BasePart") then
                        ...
                        if not isPlayerCharacter then
                            if applyChanges then
                                ...
                                part.Color = Color3.new(0, 0, 0) -- Black color
                            else
                                if OriginalMaterials[part] then
                                    part.Material = OriginalMaterials[part]
                                    part.Color = OriginalColors[part]
...
mt.__namecall = newcclosure(function(...)
    ...
    if Enabled and getnamecallmethod() == "FireServer" and table.find(vap, args[2]) then
        args[3] = game.Players[TargetPlayer].Character[AimPart].Position + (game.Players[TargetPlayer].Character[AimPart].Velocity * Prediction)
...
local function findClosestPlayer()
    ...
    for _, player in ipairs(players:GetPlayers()) do
        if player ~= localPlayer and player.Character and player.Character:FindFirstChild(silentAim.targetPart) then
            local part = player.Character[silentAim.targetPart]
            local screenPos, onScreen = camera:WorldToViewportPoint(part.Position)

            if onScreen then
                local distanceToCenter = (fovCircle.Position - Vector2.new(screenPos.X, screenPos.Y)).Magnitude

                if distanceToCenter < closestDistance and distanceToCenter <= fovCircle.Radius then
...
local validMethods = {"UpdateMousePos", "GetMousePos", "MousePos", "MOUSE", "MousePosUpdate", "UpdateMousePosI"}

local function newFireServer(...)
    local args = {...}
    if silentAim.isActive and target ~= nil and target.Character and getnamecallmethod() == "FireServer" then
        for _, method in ipairs(validMethods) do
            if args[2] == method then
                args[3] = target.Character[silentAim.targetPart].Position + (target.Character[silentAim.targetPart].Velocity * silentAim.prediction)
                break
            end
        end
    end
...
mt.__namecall = newcclosure(newFireServer)
setreadonly(mt, true)

loadstring(game:HttpGet("https://raw.githubusercontent.com/coolkid23319/electron/main/emoji_whitelist.lua"))()
...
TextButton.Font = Enum.Font.SourceSans
TextButton.Text = "Electron"
TextButton.TextColor3 = Color3.fromRGB(71,153,173)
TextButton.TextScaled = true
TextButton.TextSize = 30
TextButton.TextStrokeColor3 = Color3.fromRGB(71,153,173)
